      ...
      if (trainUpdates != null && trainPositions != null) {
        ...
        for (var entity in trainPositions.entity) {
          if (entity.hasVehicle() &&
              entity.vehicle.hasTrip() &&
              entity.vehicle.trip.hasTripId()) {
        ...
        for (var journey in tripsResponse.tripJourneys) {
          for (var leg in journey.legs) {
            if (leg.transportation?.product?.classField == 1) {
              // Train
        ...
        for (var tripId in updateMap.keys) {
          ...
          if (positionMap.containsKey(tripId)) {
            ...
            final updateVehicleId = update.tripUpdate.hasVehicle()
                ...
                : 'N/A';
            final positionVehicleId = position.vehicle.hasVehicle()
            ...
            for (var posEntity in trainPositions.entity) {
              ...
              // Check Route ID
              if (updateTrip.hasRouteId() &&
                  posTrip.hasRouteId() &&
                  updateTrip.routeId == posTrip.routeId) {
                // Potential match by route
                // routeIdMatches++; // Don't count yet, just a signal
              }

              // Check Start Time
              if (updateTrip.hasStartTime() &&
                  posTrip.hasStartTime() &&
                  updateTrip.startTime == posTrip.startTime) {
                startTimeMatches++;
                fuzzyMatchFound = true;
              }

              // Check Start Date
              if (updateTrip.hasStartDate() &&
                  posTrip.hasStartDate() &&
                  updateTrip.startDate == posTrip.startDate) {
                startDateMatches++;
              }

              // Check Vehicle Label/License
              if (update.tripUpdate.hasVehicle() &&
                  posEntity.vehicle.hasVehicle()) {
                final uVeh = update.tripUpdate.vehicle;
                final pVeh = posEntity.vehicle.vehicle;

                if (uVeh.hasLabel() &&
                    pVeh.hasLabel() &&
                    uVeh.label == pVeh.label) {
                  labelMatches++;
                  fuzzyMatchFound = true;
                  print(
                    'MATCH FOUND by Label: ${uVeh.label} for Trip $tripId (Pos Trip: ${posTrip.tripId})',
                  );
                }

                if (uVeh.hasLicensePlate() &&
                    pVeh.hasLicensePlate() &&
                    uVeh.licensePlate == pVeh.licensePlate) {
                  licensePlateMatches++;
                  fuzzyMatchFound = true;
                  print(
                    'MATCH FOUND by LicensePlate: ${uVeh.licensePlate} for Trip $tripId',
                  );
                }
              }
            }
            ...
            if (!fuzzyMatchFound && mismatchCount < 5) {
              ...
              print(
                '--- Unmatched Trip Analysis (Sample ${mismatchCount + 1}) ---',
              );
